...
class PostgresStore:
    """Long-term memory store backed by Postgres.

    Uses the same schema/behavior as SqliteStore but stores in the central DB so
    it survives Render restarts/deploys.
    """
        ...
        try:
            with conn:
                ...
                row = conn.execute(
                    ...
                    SELECT id, project, category, memory_key, value_json, tags_json, source, created_at, updated_at, pinned
                    FROM long_term_memories
                    WHERE project=%s AND category=%s AND memory_key=%s
                    """,
                    (project_n, category_n, key_n),
                ).fetchone()
        finally:
    ...
    def search(
        ...
        *,
        ...
        if query:
            q = f"%{query.strip().lower()}%"
            where.append(
                "(lower(memory_key) LIKE %s OR lower(value_json) LIKE %s OR lower(tags_json) LIKE %s)"
            )
            params.extend([q, q, q])
...
def get_long_term_store() -> SqliteStore | PostgresStore:
    global _STORE
    if pg.enabled():
        pg.get_pg_store()  # ensure schema
        logger.info("Initialized long-term memory store in Postgres.")
...
__all__ = ["MemoryRecord", "SqliteStore", "PostgresStore", "get_long_term_store"]
